...
int main(void)
{
	...
	printf("BubbleSort_Time: %.10f\n\n",(double)(BubbleSearchTime/CLOCKS_PER_SEC)); //print time of execution
	...
	printf("\nAfter sorting: %s\n", &str); //print arr after sort
	fprintf(file2, "\nAfter sorting %s\n", &str);
	printf("SelectionSort_Time : %.10f\n\n",(double)(SelectionSearchTime/CLOCKS_PER_SEC));
	...
	printf("\nAfter sorting: %s", &str); //print arr after sort
	...
	printf ("Closing SortedValues: ");
   		if (fclose (file2) == EOF) //check, is a file2 closed or not
		  printf ("Error\n");
   		else
		  {
		  printf ("Done\n");
