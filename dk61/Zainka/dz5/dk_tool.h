...
{
	    int i, step = polochka -> current_size / 2;
	while (step > 0)
    {
      for (i = 0; i < (polochka->current_size - step); i++)
        {
      	  int j = i;
          while (j >= 0 && area( &polochka->Piramida[j] ) < area( &polochka->Piramida[j + step] ) )                        
            {
        		swap( &polochka->Piramida[j], &polochka->Piramida[j + step] );
                j--; 
            }
		}
    step /= 2; 
