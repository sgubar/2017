...
void Selection_Sort_List(spisok *the_spisok)
{
	    ...
	    for (theIn = 0; theIn < the_spisok->current_size; theIn++)
    {
        max = theIn;
        for (theOut = theIn+1; theOut < the_spisok->current_size; theOut++)
        {
            if ( Square( &the_spisok->kolas[theOut] ) < Square( &the_spisok->kolas[max] ) )
                max = theOut;
		}
            if ( max != theIn)
				peremen( &the_spisok->kolas[theIn], &the_spisok->kolas[max]);
    }
}
...
{
	return 3.14159*aCircle->r * aCircle->r;
}
