...
int LT_ExtractCharNode(CharList *aList,int anIndex)
{
    ...
    if(NULL != aList)
    {
        if(anIndex == 0)
        {
        aList->head = theNode->next;  //a(head) = b->c
        }else if(anIndex != aList->count)
            {
                theNode->prev->next = NULL; //b->NULL
                theNode->prev->next = theNode->next; //a->c
            }else
        {
        aList->tail->prev = NULL;  //b->NULL
        }
    }
