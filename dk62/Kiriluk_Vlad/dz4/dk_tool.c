...
void BubbleSortShelf(MyShelf *theShelf, int aSize)
{
	...
	for (theOut; theOut > 1; theOut --)
	{
		...
		{
			if (theShelf->Books[theIn].Name_Book > theShelf->Books[theIn+ 1].Name_Book )
			{
				BookDescription *timeBook = theShelf->Books[theIn].Name_Book;
				theShelf->Books[theIn].Name_Book = theShelf->Books[theIn + 1].Name_Book;
                theShelf->Books[theIn + 1].Name_Book = timeBook;
			}
		}
	}
}
