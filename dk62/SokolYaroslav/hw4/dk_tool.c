	...
	if(NULL != aFigure )
	{
		if(aFigure -> current_size < aFigure->size)
		{
			...
			{ 
				...
				for(i=0; i<5; i++)
				{
					theFigure->points[i].x = rand()%10000+1;
					theFigure->points[i].y = rand()%10000+1;
				}
...
{   
	...
	{ 
		for(j=0; j<i; j++) 
	{ 
		if(square(&aFigure->chotirikytnik[j])>square(&aFigure->chotirikytnik[j+1])) 
		{ 
		Figure tmp=aFigure->chotirikytnik[j]; 
		aFigure->chotirikytnik[j]=aFigure->chotirikytnik[j+1]; 
		aFigure->chotirikytnik[j+1]=tmp; 
        } 
	} 
	} 
}
