...
{   
	for(int i=myfigura->Flag-1; i>1; i--) 
	{ 
		for(int j=0; j<i; j++) 
	{ 
		if(Ploshcha(&myfigura->quadrangle[j])>Ploshcha(&myfigura->quadrangle[j+1])) 
		{ 
		figura tmp=myfigura->quadrangle[j]; 
		myfigura->quadrangle[j]=myfigura->quadrangle[j+1]; 
		myfigura->quadrangle[j+1]=tmp; 
        } 
	} 
	} 
}
int BinarySearch(spisok_figur *myfigura)
{
	...
	{
		middle = (Left + Right)/2;
		if (Ploshcha(&myfigura->quadrangle[middle]) == Value)
		{
			int Result = middle;
			return Result+1;
		}
...
{
	figura theTmp = myfigura->quadrangle[aLeftIndex];
	myfigura->quadrangle[aLeftIndex] = myfigura->quadrangle[aRightIndex];
	myfigura->quadrangle[aRightIndex] = theTmp;
}
int mediana(spisok_figur *myfigura, int aLeftIndex, int aRightIndex)
{
	...
	{
		swap(myfigura, aLeftIndex, theCenter);
	}

	if (Ploshcha(&myfigura->quadrangle[aLeftIndex]) > Ploshcha(&myfigura->quadrangle[aRightIndex]))
	{
		swap(myfigura, aLeftIndex, aRightIndex);
	}

	if (Ploshcha(&myfigura->quadrangle[theCenter]) > Ploshcha(&myfigura->quadrangle[aRightIndex]))
	{
		swap(myfigura, theCenter, aRightIndex);
	}
	
	swap(myfigura, theCenter, aRightIndex - 1);
	
	
	return Ploshcha(&myfigura->quadrangle[aRightIndex-1]);;
}
...
int m_partitionIt(spisok_figur *myfigura, int aLeftIndex, int aRightIndex, int aPivot)
{
	...
	while (1)
	{
		...
		while (Ploshcha(&myfigura->quadrangle[++theLeft]) < aPivot);
	
		// search the lowest element
		while (Ploshcha(&myfigura->quadrangle[--theRight]) > aPivot);
		...
		else
		{
			...
			swap(myfigura, theLeft, theRight);
		}
	}

	//lets to swap elements
	swap(myfigura, theLeft, aRightIndex-1);
...
void quickSort2(spisok_figur *myfigura, int aLeftIndex, int aRightIndex)
{
	...
	if (theSize <= 3)
	{
		m_manualSort(myfigura, aLeftIndex, aRightIndex);
	}
	else
	{
		int thePivot = mediana(myfigura, aLeftIndex, aRightIndex);
		int thePartitionIndex = m_partitionIt(myfigura, aLeftIndex, aRightIndex, thePivot);
		
		//left part sorting
		quickSort2(myfigura, aLeftIndex, thePartitionIndex - 1);
		
		//right part sorting
		quickSort2(myfigura, thePartitionIndex + 1, aRightIndex);
	}
}
...
void m_manualSort(spisok_figur *myfigura, int aLeftIndex, int aRightIndex)
{
	...
	else
	{
		...
		if (Ploshcha(&myfigura->quadrangle[aLeftIndex]) > Ploshcha(&myfigura->quadrangle[aRightIndex-1]))
	     {
		swap(myfigura, aLeftIndex, aRightIndex-1);
	     }
		...
		if (Ploshcha(&myfigura->quadrangle[aRightIndex - 1]) > Ploshcha(&myfigura->quadrangle[aRightIndex]))
		{
			swap(myfigura, aRightIndex - 1, aRightIndex);
		}
	}
}
