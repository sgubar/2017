...
void Shell_Sort_List(pyramidList *aPyramidList)
{
	int i, step = aPyramidList->current_size / 2;
	while (step > 0)
	{
		for (i = 0; i < (aPyramidList->current_size - step); i++)
		{
			int j = i;
			while (j >= 0 && square(&aPyramidList->arrayofPyramids[j]) < square(&aPyramidList->arrayofPyramids[j + step]))
			{
				swap(&aPyramidList->arrayofPyramids[j], &aPyramidList->arrayofPyramids[j + step]);
				j--;
			}
		}
		step /= 2;
