...
int main(int argc, const char *argv[])
{
	...
	switch(arg)
	{
			...
			while(true)
			{
				do{
					...
					do{
						...
						fflush(stdin);
						val = getIntValue();
						tmp = search(theTree->root, val);
					}while(tmp);
					
					insertElement(theTree, val);
					printf("Value added!\n");
					...
					if(arg == 'd' || arg == 'D')
					{
						do{
							...
							do{
								fflush(stdin);
								i = getIntValue();
								tmp = search(theTree->root, i);
								if(!tmp)
									printf("No such element.\nRetry: ");
							}while(!tmp);
							
							delValElement(theTree, theTree->root, i);
		...
		case 'T':
		{
			...
			delValElement(theTree, theTree->root, theTree->root->value);
			printf("\nNew value: %i\n", theTree->root->value);
